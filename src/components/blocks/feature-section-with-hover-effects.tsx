...
const Feature = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex flex-col lg:border-r py-6 px-4 sm:py-10 sm:px-10 relative group/feature border-slate-200 active:bg-blue-50/30 md:hover:bg-blue-50/30 transition-colors duration-200 dark:border-slate-700 dark:active:bg-blue-900/20 dark:md:hover:bg-blue-900/20",
        (index === 0 || index === 4) && "lg:border-l",
        index < 4 && "lg:border-b"
      )}
    >
      {index < 4 && (
        <div className="opacity-0 group-active/feature:opacity-100 md:group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-t from-blue-50/50 to-transparent pointer-events-none dark:from-blue-900/20" />
      )}
      {index >= 4 && (
      ...
      <div className="text-lg font-bold mb-2 relative z-10">
        <div className="absolute left-0 inset-y-0 h-6 group-active/feature:h-8 md:group-hover/feature:h-8 w-1 rounded-tr-full rounded-br-full bg-slate-300 group-active/feature:bg-blue-500 md:group-hover/feature:bg-blue-500 transition-all duration-200 origin-center dark:bg-slate-600" />
        <span className="group-active/feature:translate-x-2 md:group-hover/feature:translate-x-2 transition duration-200 inline-block text-slate-900 dark:text-white">
          {title}
        </span>
      </div>
      <p className="text-sm text-slate-600 w-full sm:max-w-xs relative z-10 dark:text-slate-400">
        {description}
      </p>
    </motion.div>
  );
