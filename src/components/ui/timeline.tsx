...
export const Timeline = ({ data, className }: TimelineProps) => {
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "w-full font-sans text-slate-900 dark:text-white",
        className
      )}
    >
      <div ref={ref} className="relative mx-auto max-w-5xl pb-10 md:pb-20">
          ...
          >
            ...
            >
              <div className="relative pl-20">
                <div className="absolute left-3 top-1 h-10 w-10 rounded-full border border-slate-300 bg-slate-100 backdrop-blur dark:border-slate-600 dark:bg-slate-800">
                  <div className="m-3 h-4 w-4 rounded-full bg-blue-500" />
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-xs uppercase tracking-[0.4em] text-slate-500 dark:text-slate-400">
                    {item.period}
                  </span>
                  <h3 className="text-3xl font-semibold text-slate-900 dark:text-white">
                    {item.project}
                  </h3>
                  <span className="text-sm text-slate-600 dark:text-slate-400">{item.team}</span>
                </div>
              </div>
            </motion.div>
        ...
        <div
          ...
          className="absolute left-8 top-0 w-[2px] overflow-hidden bg-gradient-to-b from-transparent via-slate-300 to-transparent [mask-image:linear-gradient(to_bottom,transparent_0%,black_10%,black_90%,transparent_100%)] dark:via-slate-600"
        >
          <motion.div
